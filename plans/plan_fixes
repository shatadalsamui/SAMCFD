...
- [x] **Engine Liquidation Math is Incorrect**
  ...
  - **Status:** ✅ Fixed and fully integrated. Liquidation logic now uses Trade and calculate_pnl for all checks, and is called only on open positions via periodic monitoring.
  - **Fix Details:**
    - Refactored `check_liquidation` in `liquidations.rs` to accept full Trade object and use `calculate_pnl`.
    - Integrated liquidation checks into periodic monitoring task in `stop_loss_take_profit.rs`.
    - Removed premature liquidation checks from order creation flow in `processor.rs`.
    - Orders now match first, then open positions are monitored for liquidation.
  - **Test Results:**
    - ✅ 4-payload test passed: User A earned +1000 PnL (buy at 105000, sell at 106000).
    - ✅ No premature liquidations on new orders.
    - ✅ Position tracking and PnL calculation working correctly.
  - **Artifacts:**
    - Refactored `liquidations.rs`, `processor.rs`, and `stop_loss_take_profit.rs`.
    - Test logs showing correct matching, position tracking, and PnL:
      ```
      Order 56e7b72b-a6c7-4796-986d-99afd5aab4c8 filled. Closing long position. Entry: 105000, Close: 106000, PnL: 1000
      ```
